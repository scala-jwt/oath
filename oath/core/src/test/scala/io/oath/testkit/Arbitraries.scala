...
trait Arbitraries {
  private lazy val genPositiveFiniteDuration        = Gen.posNum[Long].map(Duration.fromNanos)
  private lazy val genPositiveFiniteDurationSeconds = Gen.posNum[Int].map(x => (x + 1).seconds)
